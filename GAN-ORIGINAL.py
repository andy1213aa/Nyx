...
class GAN():
    ...
    def generator_loss(self, fake_logit, real_data, fake_data_by_real_parameter):
        ...
        #l2_norm = tf.reduce_mean(tf.norm(tensor = (fake_data_by_real_parameter-real_data[1][0]), ord='euclidean'))
    ...
    def train_generator(self, real_data):
        with tf.GradientTape() as tape:
            ...
            fake_loss = self.generator_loss(fake_logit, real_data, fake_data_by_real_parameter)
            gLoss = fake_loss# + self.L2_coefficient*l1_loss
...
for num_units in HP_NUM_UNITS.domain.values:
    for bn_unit in HP_BN_UNITS.domain.values:
        ...
        GANs = GAN(length = 16, width = 16, height = 1, batchSize = 64, epochs = 2000, dataSetDir = r'E:\NTNU1-2\Nyx\NyxDataSet16_16', hparams = hparams, logdir = dirs+'\\'+session_num)
        GANs.train_wgan()
        num+=1
