...
class GAN():
    def __init__(self, length, width, height, batchSize, epochs, dataSetDir,hparams, logdir):
        ...
        self.genOptimizer = keras.optimizers.RMSprop(lr = 0.005, 
                                                        clipvalue = 10.0, 
                                                        decay = 1e-8)
        #self.genOptimizer = keras.optimizers.Adam(lr = 0.0005, beta_1 = 0.5, beta_2 = 0.9)                                            
        self.decod = self.decoder()
        self.decoderOptimizer = keras.optimizers.Adam(lr = 0.02, beta_1 = 0.5, beta_2 = 0.9)
    ...
    def generator_loss(self, fake_logit, real_data, fake_data_by_real_parameter):
        ...
        l2_norm = tf.norm(tensor = (real_data[1][1]-fake_data_by_real_parameter), ord='euclidean')
        #l2_norm = 0
    ...
    def train_wgan(self):
        ...
        for epoch in range(1, self.epochs+1):
            for step, real_data in enumerate(train_data):
                ...
                l2 = tf.norm(tensor = real_data[1][1]-predi_data,  ord='euclidean')
                with summary_writer.as_default():
                    hp.hparams(hparams)
                    tf.summary.scalar('Mean square error', l2, epoch)
            ...
            print(f'Epoch: {epoch} G Loss: {g_loss} D loss: {d_loss} GP Loss {gp} MSE: {l2}' )
